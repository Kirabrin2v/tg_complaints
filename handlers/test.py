...
confirm_complaint_message = load_html("confirm_complaint.html")
wait_end_dialogue_message = load_html("wait_end_dialogue.html")
choice_edit_complaint_message = load_html("choice_edit_complaint.html")
wait_edit_complaint_message = load_html("wait_edit_complaint.html")
...
not_fullness_complaint_message = load_html(
    "accept_complaint/errors/not_fullness_complaint.html"
)

complaint_exist_message = load_html(
    "accept_complaint/errors/active_complaint_exist.html"
)
success_send_complaint_message = load_html(
    "accept_complaint/success_send_complaint.html"
)
...
async def accept_complaint_type_handler(
    ...
    if "complaint" not in context.user_data:
        ...
        search_next_handler = SearchNextHandler(
            handlers=handlers_sequence, message_info=message_info, fallback=fallback
        )
...
async def accept_nick_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if re.fullmatch(REG_NICK, nick):
        context.user_data["complaint"]["nick"] = update.message.text
    ...
    if len(text) >= 3:
        ...
        if re.fullmatch(REG_DATE, date_event):
            if re.fullmatch(REG_TIME, time_event):
                date = get_datetime(f"{date_event} {time_event}")
                if date:
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await remove_reply_keyboard(tg_id=update.effective_chat.id)
...
collect_complaint_info_handler = ConversationHandler(
    ...
    fallbacks=[
        ...
        MessageHandler(filters.Regex("^Отменить жалобу$"), cancel),
        CallbackQueryHandler(
